...
<style>
    ...
    div[data-baseweb="input"] > div, div[data-baseweb="select"] > div {
        ...
        border: 1px solid #CBD5E1 !important; box-shadow: inset 0 1px 2px rgba(0,0,0,0.02);
    }
    ...
    .stButton>button {
        ...
        color: #FFFFFF !important; font-weight: bold; letter-spacing: 1px;
        border-radius: 10px; border: none; width: 100%;
        box-shadow: 0 4px 8px -1px rgba(0, 91, 171, 0.3); transition: all 0.2s ease;
    }
    .stButton>button:active { transform: scale(0.98); box-shadow: 0 2px 4px -1px rgba(0, 91, 171, 0.3); }
...
if menu == "📝 記録する":
    ...
    st.markdown(f"<div style='background-color: #FFFFFF; padding: 10px; border-radius: 8px; color: #0A2B56; font-size: 1.05rem; text-align: center; margin-top: 10px; margin-bottom: 20px; font-weight: bold; border: 1px dashed #CBD5E1;'>🕒 確認： {disp_in} 〜 {disp_out}</div>", unsafe_allow_html=True)
...
# ---------------------------------------------------------
    ...
    def render_premium_cards(agg):
        ...
        for i, row in top_rows.iterrows():
            ...
            html += f"<div style='font-size: 2rem; font-weight: 900; color: #0F172A; margin-bottom: 12px;'>{name} <span style='font-size: 1rem; font-weight: 600; color: #64748B;'>さん</span></div>"
